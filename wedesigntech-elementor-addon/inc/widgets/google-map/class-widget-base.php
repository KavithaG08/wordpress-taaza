...
class WeDesignTech_Widget_Base_Google_Map {
	...
	public function create_elementor_controls($elementor_object) {
		...
		// Markers
			...
			// Custom Heading
				...
				$elementor_object->add_control( 'show_custom_heading', array(
					'label'        => esc_html__( 'Show Heading', 'wdt-elementor-addon' ),
				...
				$elementor_object->add_control( 'additional_content', array (
				...
				$elementor_object->add_control( 'marker_additional_type', array(
					'label'     => esc_html__( 'Additional Detail Type', 'wdt-elementor-addon' ),
					'type'      => \Elementor\Controls_Manager::SELECT,
					'default'   => 'type1',
					'options'   => array(
				...
				$repeater->add_control( 'window_image', array (
					'label' => esc_html__( 'Info Window - Image', 'wdt-elementor-addon' ),
	...
	public function map_markers( $markers ) {
		...
		foreach( $markers as $index => $marker ) {
			...
			$obj = array(
				...
				'title'                              => $marker['title'],
				'desc'                               => $marker['desc'],
				'window_image'                       => $marker['window_image'],
	...
	public function render_html($widget_object, $settings) {
		...
		$output .= '<div '.wedesigntech_html_output($widget_object->get_render_attribute_string( 'wrapper' )).'>';
			...
			if(isset($settings['marker_additional_details']) && $settings['marker_additional_details'] == 'true') {
				if(is_array($map_markers_list) && !empty($map_markers_list)) {
					$output .= '<div class="wdt-google-map-marker-content-wrapper accordion">';

						if (!empty($settings['additional_title']) || !empty($settings['additional_subtitle']) || !empty($settings['additional_content'])) {
							$output .= '<div class="wdt-google-map-heading-title-wrapper">';
								if (!empty($settings['additional_subtitle'])) {
									$output .= '<div class="wdt-heading-sub-title-wrapper">';
										$output .= '<span class="wdt-heading-title">'.$settings['additional_subtitle'].'</span>';
									$output .= '</div>';
								}
								if (!empty($settings['additional_title'])) {
									$output .= '<div class="wdt-heading-title-wrapper">';
										$output .= '<h2 class="wdt-heading-title">'.$settings['additional_title'].'</h2>';
									$output .= '</div>';
								}
								if (!empty($settings['additional_content'])) {
									$output .= '<div class="wdt-heading-content-wrapper">'.$settings['additional_content'].'</div>';
								}
							$output .= '</div>';
						}

						$output .= '<div class="wdt-google-map-marker-content-selection '.$settings['marker_additional_type'].'">';
							foreach($map_markers_list as $map_marker_list) {

								$output .= '<div class="wdt-google-map-mark-item" id="'.esc_attr($map_marker_list['key']).'">';

									if (!empty($map_marker_list['window_image']['url'])) {
										$output .= '<div class="wdt-google-map-info-window-image"><img src="'.esc_html($map_marker_list['window_image']['url']).'" /></div>';
									}

									$output .= '<div class="wdt-google-map-mark-title">';
										$output .= '<h5 class="wdt-google-mark-title">'.esc_html($map_marker_list['title']).'</h5>';
										$output .= '<span class="wdt-google-title--icon"></span>';
									$output .= '</div>';

									$output .= '<div id="wdt-google-map-marker-content-'.esc_attr($map_marker_list['key']).'" class="wdt-google-map-marker-content-item">';
										if($map_marker_list['marker_additional_details_type'] == 'template') {
											$frontend = Elementor\Frontend::instance();
											$output .= $frontend->get_builder_content($map_marker_list['marker_additional_details_template'], true);
										} else {
