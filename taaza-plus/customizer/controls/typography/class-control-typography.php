...
class Taaza_Customize_Control_Typography extends WP_Customize_Control {
	...
	public function enqueue() {
		...
		wp_enqueue_script( 'taaza-plus-typography-control', TAAZA_PLUS_DIR_URL.'customizer/controls/typography/typography.js', array( 'jquery', 'customize-base' ), TAAZA_PLUS_VERSION, true );
		wp_enqueue_style( 'taaza-plus-typography-control',  TAAZA_PLUS_DIR_URL.'customizer/controls/typography/typography.css', null, TAAZA_PLUS_VERSION );

		$typo_localize = array(
			'inherit' => esc_html__( 'Inherit', 'taaza-plus' ),
			'100'     => esc_html__( 'Thin 100', 'taaza-plus' ),
			'200'     => esc_html__( 'Extra-Light 200', 'taaza-plus' ),
			'300'     => esc_html__( 'Light 300', 'taaza-plus' ),
			'400'     => esc_html__( 'Normal 400', 'taaza-plus' ),
			'500'     => esc_html__( 'Medium 500', 'taaza-plus' ),
			'600'     => esc_html__( 'Semi-Bold 600', 'taaza-plus' ),
			'700'     => esc_html__( 'Bold 700', 'taaza-plus' ),
			'800'     => esc_html__( 'Extra-Bold 800', 'taaza-plus' ),
			'900'     => esc_html__( 'Ultra-Bold 900', 'taaza-plus' ),
			'normal'  => esc_html__( 'Normal', 'taaza-plus' ),
			'normal'  => esc_html__( 'Normal', 'taaza-plus' ),
			'bold'    => esc_html__( 'Bold', 'taaza-plus' ),
			''        => esc_html__('Default', 'taaza-plus' ),
		);

		wp_localize_script( 'taaza-plus-typography-control', 'wdtPlusTypoObject', $typo_localize );
	}
	...
	public function get_custom_fonts() {
		$fonts = apply_filters( 'taaza_customizer_custom_fonts', array() );
		return $fonts;
	}
	...
	public function get_google_fonts() {
		...
		$google_fonts_file = TAAZA_PLUS_DIR_PATH . 'customizer/controls/typography/google-fonts.json';
	...
	public function to_json() {
		...
		if ( ! is_array( $choices ) || empty( $choices ) ) {
			...
			$this->choices = array(
				'font_family'     => esc_html__( 'Font Family', 'taaza-plus'),
				'font_weight'     => esc_html__( 'Font Weight', 'taaza-plus'),
				'text_transform'  => esc_html__( 'Text Transform', 'taaza-plus'),
				'text_align'      => esc_html__( 'Text Align', 'taaza-plus'),
				'text_decoration' => esc_html__( 'Text Decoration', 'taaza-plus'),
				'font_style'      => esc_html__( 'Font Style', 'taaza-plus'),
				'font_size'       => esc_html__( 'Font Size', 'taaza-plus'),
				'line_height'     => esc_html__( 'Line Height', 'taaza-plus'),
				'letter_spacing'  => esc_html__( 'Letter Spacing', 'taaza-plus'),
			);
		}
		...
		<div class="wrapper">
			...
			<div class="full-width">
				...
				<select class="font-family">
					<option value=""><?php esc_html_e( 'Inherit', 'taaza-plus' );?></option>
					<optgroup label="<?php esc_html_e( 'Other System Fonts', 'taaza-plus' );?>">
					...
					<# if( _.isEqual( data.has_custom_fonts, {'item':'yes' } ) ) { #>
						<optgroup label="<?php esc_html_e( 'Custom Fonts', 'taaza-plus' );?>">
					...
					<optgroup label="<?php esc_html_e( 'Google', 'taaza-plus' );?>">
				...
				<# if( !_.isUndefined( data.choices['font_weight'] ) ) { #>
					<div class="one-half">
						...
						<select class="font-weight">
							<option value="inherit"><?php esc_html_e( 'Inherit', 'taaza-plus' );?></option>
						</select>
					</div>
				<# } #>
				...
				<# if( !_.isUndefined( data.choices['font_style'] ) ) { #>
					<div class="one-half">
						...
						<select class="font-style">
							...
							<option value="normal"<# if ( 'none' === data.value['font-style'] ) { #>selected<# } #>><?php esc_attr_e( 'Normal', 'taaza-plus' ); ?></option>
							<option value="italic"<# if ( 'italic' === data.value['font-style'] ) { #>selected<# } #>><?php esc_attr_e( 'Italic', 'taaza-plus' ); ?></option>
							<option value="oblique"<# if ( 'oblique' === data.value['font-style'] ) { #>selected<# } #>><?php esc_attr_e( 'Oblique', 'taaza-plus' ); ?></option>
							<option value="inherit"<# if ( 'inherit' === data.value['font-style'] ) { #>selected<# } #>><?php esc_attr_e( 'Inherit', 'taaza-plus' ); ?></option>
						</select>
					</div>
				<# } #>
			</div>
			...
			<# if( !_.isUndefined( data.choices['text_transform'] ) ) { #>
				<div class="full-width">
					...
					<select class="text-transform">
						...
						<option value="none"<# if ( 'none' === data.value['text-transform'] ) { #>selected<# } #>><?php esc_attr_e( 'None', 'taaza-plus' ); ?></option>
						<option value="capitalize"<# if ( 'capitalize' === data.value['text-transform'] ) { #>selected<# } #>><?php esc_attr_e( 'Capitalize', 'taaza-plus' ); ?></option>
						<option value="uppercase"<# if ( 'uppercase' === data.value['text-transform'] ) { #>selected<# } #>><?php esc_attr_e( 'Uppercase', 'taaza-plus' ); ?></option>
						<option value="lowercase"<# if ( 'lowercase' === data.value['text-transform'] ) { #>selected<# } #>><?php esc_attr_e( 'Lowercase', 'taaza-plus' ); ?></option>
						<option value="initial"<# if ( 'initial' === data.value['text-transform'] ) { #>selected<# } #>><?php esc_attr_e( 'Initial', 'taaza-plus' ); ?></option>
						<option value="inherit"<# if ( 'inherit' === data.value['text-transform'] ) { #>selected<# } #>><?php esc_attr_e( 'Inherit', 'taaza-plus' ); ?></option>
					</select>
				</div>
			<# } #>
				...
				<# if( !_.isUndefined( data.choices['text_align'] ) ) { #>
					<div class="one-half">
						...
						<select class="text-align">
							...
							<option value="unset"<# if ( 'unset' === data.value['text-align'] ) { #>selected<# } #>><?php esc_attr_e( 'Unset', 'taaza-plus' ); ?></option>
							<option value="left"<# if ( 'left' === data.value['text-align'] ) { #>selected<# } #>><?php esc_attr_e( 'Left', 'taaza-plus' ); ?></option>
							<option value="center"<# if ( 'center' === data.value['text-align'] ) { #>selected<# } #>><?php esc_attr_e( 'Center', 'taaza-plus' ); ?></option>
							<option value="right"<# if ( 'right' === data.value['text-align'] ) { #>selected<# } #>><?php esc_attr_e( 'Right', 'taaza-plus' ); ?></option>
							<option value="justify"<# if ( 'justify' === data.value['text-align'] ) { #>selected<# } #>><?php esc_attr_e( 'Justify', 'taaza-plus' ); ?></option>
							<option value="inherit"<# if ( 'inherit' === data.value['text-align'] ) { #>selected<# } #>><?php esc_attr_e( 'Inherit', 'taaza-plus' ); ?></option>
						</select>
					</div>
				<# } #>
				...
				<# if( !_.isUndefined( data.choices['text_decoration'] ) ) { #>
					<div class="one-half">
						...
						<select class="text-decoration">
							...
							<option value="none"<# if ( 'none' === data.value['text-decoration'] ) { #>selected<# } #>><?php esc_attr_e( 'None', 'taaza-plus' ); ?></option>
							<option value="underline"<# if ( 'underline' === data.value['text-decoration'] ) { #>selected<# } #>><?php esc_attr_e( 'Underline', 'taaza-plus' ); ?></option>
							<option value="overline"<# if ( 'overline' === data.value['text-decoration'] ) { #>selected<# } #>><?php esc_attr_e( 'Overline', 'taaza-plus' ); ?></option>
							<option value="line-through"<# if ( 'line-through' === data.value['text-decoration'] ) { #>selected<# } #>><?php esc_attr_e( 'Line Through', 'taaza-plus' ); ?></option>
							<option value="inherit"<# if ( 'inherit' === data.value['text-decoration'] ) { #>selected<# } #>><?php esc_attr_e( 'Inherit', 'taaza-plus' ); ?></option>
						</select>
					</div>
				<# } #>
			</div>
