...
if( !function_exists('taaza_archive_blog_post_misc_defaults') ) {
    function taaza_archive_blog_post_misc_defaults() {
        $defaults = array(
...
if( !function_exists('taaza_archive_blog_post_params_default') ) {
    function taaza_archive_blog_post_params_default() {
        $params = array(
            'enable_post_format'   	 => 0,
            'enable_video_audio' 	 => 0,
            'enable_gallery_slider'  => 0,
            'archive_post_elements'  => array( 'feature_image', 'date', 'category', 'title', 'content', 'read_more' ),
            'archive_meta_elements'  => array( 'author', 'category', 'tag', 'date', 'comment' ),
            'archive_readmore_text'  => esc_html__('Read More', 'taaza'),
...
if( !function_exists('taaza_archive_blog_post_defaults_filter') ) {
    function taaza_archive_blog_post_defaults_filter() {
        $defaults = taaza_archive_blog_post_defaults();
        return apply_filters( 'taaza_archive_post_cmb_class', $defaults );
    }
}

if( !function_exists('taaza_archive_blog_post_misc_defaults_filter') ) {
    function taaza_archive_blog_post_misc_defaults_filter() {
        $defaults = taaza_archive_blog_post_misc_defaults();
        return apply_filters( 'taaza_archive_post_hld_class', $defaults );
    }
}

if( !function_exists('taaza_archive_blog_post_params') ) {
    function taaza_archive_blog_post_params() {
        $params = taaza_archive_blog_post_params_default();
        return apply_filters( 'taaza_archive_blog_post_params', $params );
    }
}

if( !function_exists('taaza_get_archive_post_combine_class') ) {
	function taaza_get_archive_post_combine_class() {

        $blog_defaults = taaza_archive_blog_post_defaults_filter();
        ...
        return apply_filters( 'taaza_get_archive_post_combine_class', implode( ' ', $combine_class ) );
	}
}

if( !function_exists('taaza_get_archive_post_holder_class') ) {
	function taaza_get_archive_post_holder_class() {

        $blog_defaults = taaza_archive_blog_post_defaults_filter();
        $blog_misc_defaults = taaza_archive_blog_post_misc_defaults_filter();
        ...
        return apply_filters( 'taaza_get_archive_post_holder_class', implode( ' ', $holder_class ) );
	}
}

if( !function_exists('taaza_get_archive_post_style') ) {
	function taaza_get_archive_post_style() {
		...
		return apply_filters( 'taaza_get_archive_post_style', $post_style );
	}
}

if( !function_exists('taaza_get_archive_post_column') ) {
	function taaza_get_archive_post_column() {
		...
		return apply_filters( 'taaza_get_archive_post_column', $post_columns );
	}
}

add_filter('post_class', 'taaza_archive_set_post_class', 10, 3);
if( !function_exists('taaza_archive_set_post_class') ) {
    function taaza_archive_set_post_class( $classes, $class, $post_id ) {
        ...
        if( is_post_type_archive('post') || is_search() || is_category() || is_tag() || is_home() || is_author() || is_year() || is_month() || is_day() || is_time() || is_tax('post_format') || ( defined('DOING_AJAX') && DOING_AJAX ) ) {
            ...
            $blog_params = taaza_archive_blog_post_params();
...
add_action( 'taaza_after_main_css', 'taaza_blog_enqueue_css', 10 );
if( !function_exists( 'taaza_blog_enqueue_css' ) ) {
	function taaza_blog_enqueue_css() {
		wp_enqueue_style( 'wdt-blog', get_theme_file_uri('/modules/blog/assets/css/blog.css'), false, TAAZA_THEME_VERSION, 'all');
		...
		wp_enqueue_style( 'jquery-bxslider', get_theme_file_uri('/modules/blog/assets/css/jquery.bxslider.css'), false, TAAZA_THEME_VERSION, 'all' );
	}
}

add_action( 'taaza_before_enqueue_js', 'taaza_blog_enqueue_js' );
if( !function_exists( 'taaza_blog_enqueue_js' ) ) {
	function taaza_blog_enqueue_js() {
...
if( !function_exists( 'after_blog_post_content_pagination' ) ) {
    function after_blog_post_content_pagination() {
    	...
    	$pagination_template = taaza_archive_blog_post_params();
    	$pagination_template = $pagination_template['archive_blog_pagination'];

        echo apply_filters( 'taaza_blog_archive_pagination', taaza_get_template_part( 'pagination', 'templates/'.$pagination_template ) );
    }
    add_action( 'taaza_after_blog_post_content_wrap', 'after_blog_post_content_pagination' );
}

if( !function_exists( 'taaza_excerpt' ) ) {
	function taaza_excerpt( $limit = NULL ) {
