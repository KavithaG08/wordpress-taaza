<?php
add_action( 'taaza_after_main_css', 'breadcrumb_style' );
function breadcrumb_style() {
    wp_enqueue_style( 'taaza-breadcrumb', get_theme_file_uri('/modules/breadcrumb/assets/css/breadcrumb.css'), false, TAAZA_THEME_VERSION, 'all');
}

if( ! function_exists( 'taaza_breadcrumb_template' )  ) {
	function taaza_breadcrumb_template() {
        if ( !is_front_page() && !is_home() ) {
		    taaza_template_part( 'breadcrumb', 'templates/title' );
        }
	}

	add_action( 'taaza_breadcrumb', 'taaza_breadcrumb_template' );
}
function taaza_breadcrumb_params() {
    $params = array(
        "home"            => esc_html__('Home','taaza'),
        "home_link"       => home_url('/'),
        "delimiter"       => '<span class="breadcrumb-default-delimiter"></span>',
        "wrapper_classes" => "dark-bg-breadcrumb aligncenter"
    );
    return apply_filters( 'taaza_breadcrumb_params', $params );
}

if( ! function_exists( 'taaza_update_header_wrapper_classes' )  ) {
	function taaza_update_header_wrapper_classes($classes) {
        array_push($classes, 'wdt-dark-bg'); // For dark bg enable this line
        return $classes;
	}
	add_filter( 'taaza_header_wrapper_classes', 'taaza_update_header_wrapper_classes', 10, 1 );
}

function taaza_breadcrumb_title() {
    $title = get_the_title( taaza_get_page_id() );
	...
	if ( ( is_home() && is_front_page() ) || is_singular('attachment') ) {
        ...
        $title = esc_html__("Search Result for: ",'taaza').' '.get_search_query();
    }

    return apply_filters( 'taaza_breadcrumb_title', '<h1>'.$title.'</h1>' );
}

function taaza_breadcrumbs( $home, $separator ) {
    $output      = '';
    $breadcrumbs = array();
    ...
    if( is_search() ) {
    	$breadcrumbs[] = '<a href="javascript:void(0);">' . esc_html__('Search', 'taaza') . '</a>';
    }
    ...
    $filtered_breadcrumbs = apply_filters( 'taaza_breadcrumbs', $breadcrumbs );
