<?php
add_action( 'taaza_after_main_css', 'pagination_style' );
function pagination_style() {
    wp_enqueue_style( 'taaza-pagination', get_theme_file_uri('/modules/pagination/assets/css/pagination.css'), false, TAAZA_THEME_VERSION, 'all');
}
...
if( !function_exists( 'after_single_page_content_wp_link_pages' ) ) {
    ...
    function after_single_page_content_wp_link_pages() {
        ...
        edit_post_link( esc_html__( ' Edit ','taaza' ) );
    }

    add_action( 'taaza_after_single_page_content', 'after_single_page_content_wp_link_pages' );
}

if( !function_exists( 'taaza_pagination' ) ) {
    function taaza_pagination( $query = false, $load_more = false ) {
        ...
        $end_size = 1;
        $mid_size = 2;
        #$show_all = taaza_get_option( 'showall-pagination' );
        $dots = false;
                ...
                if( $paged < $total ){
                    $output .= '<div class="column one pager_wrapper pager_lm">';
                        $output .= '<a class="pager_load_more button button_js" href="'. get_pagenum_link( $next ) .'">';
                            ...
                            $output .= '<span class="button_label">'. esc_html__('Load more', 'taaza') .'</span>';
                        $output .= '</a>';
                    $output .= '</div>';
                }
